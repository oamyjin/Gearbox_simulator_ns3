...
namespace ns3 {
    ...
    QueueDiscItem* Level_flex::fifoEnque(QueueDiscItem* item, int index, int flowid) {
    ...
    QueueDiscItem* Level_flex::pifoEnque(QueueDiscItem* item, int flowid) {
	...
	//cout<<"pifomax"<<getPifoMaxValue()<<endl;
        ...
        if (departureRound < getPifoMaxValue() || isAllFifosEmpty()) {
            ...
            QueueDiscItem* re = pifo.Push(item, departureRound, flowid);

	    //cout<<"max0"<<getPifoMaxValue()<<endl;
		...
		if (getReloadHold() == 1 && departureround < getPifoMaxValue()){   
	        ...
	        re = fifoEnque(re, cal_index(departureround), tag.GetFlowNo());// redirect to this level's fifo
	    ...
	    QueueDiscItem* re = fifoEnque(item, cal_index(departureround),flowid);
	...
	////cout<<"flowtag"<<tag.GetFlowNo()<<endl;

	//h.FixNewFile(GetPointer(item), tag.GetUid(), "fifodeque",Simulator::Now().GetSeconds());
   ...
   int Level_flex::Reload(int k) {
	...
	int npkts;
		...
		if (defaultMaxFlag == 1){ 
			...
			setPifoMaxValue((tag0.GetDepartureRound() / term) * term + term - 1);   
		...
		if (tag0.GetDepartureRound() <= getPifoMaxValue()){
       			...
       			QueueDiscItem* re1 = pifo.Push(re, tag0.GetDepartureRound(),tag0.GetFlowNo());
			...
			if (re1 != NULL){    
				...
				int departureround = tag.GetDepartureRound();
		    		...
		    		fifoEnque(re1, cal_index(departureround), tag.GetFlowNo());    //20220102    
  ...
  int Level_flex::getFifoMaxNPackets(){
   ...
   void Level_flex::setPifoReloadHold(int hold){

	pifo.SetReloadHold(hold);
