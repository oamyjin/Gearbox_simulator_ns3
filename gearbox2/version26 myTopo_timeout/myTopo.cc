...
const int PKTSIZE = 1024; // pkt size
...
double nSamplingPeriod = 0.0001; // 0.000006115; // 0.0001
double simulator_stop_time = 0.02;// 0.0301; //0.00864049
...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		// Server
                ...
                app->Setup (ns3TcpSocket, sinkAddress, PKTSIZE, flow_input.maxPacketCount, DataRate (DATARATE)); // (TODO flow size) Ptr<Socket> socket, Address address, uint32_t packetSize, uint32_t nPackets, DataRate dataRate
 	...
 	Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow (i->first);
        ...
        if (t.destinationAddress == Ipv4Address(dstip.str().c_str())){
		        ...
		        cout << "==========" << src_No << "_" << src_port << "=============" << endl; 
                        ...
                        if (localThrou != 0){
                                lastRxTime[k_fi_pair[k]] = curTime.GetSeconds();
                        }
        }
  }
...
main (int argc, char *argv[])
{
        ...
        Config::SetDefault ("ns3::TcpSocketBase::MinRto", TimeValue(Seconds (0.000036)));//RTO=3RTT=36us
        Config::SetDefault ("ns3::TcpSocketBase::ClockGranularity", TimeValue(MilliSeconds (0.001)));//RTO=3RTT=36us
	...
	begint = clock();
        ...
        flowf.open("scratch/traffic_test.txt"); //traffic_test.txt //traffic_fct_20
	...
	flowf >> flow_num;
        ...
        for (int i = 0; i < CLIENT_CNT; i++){      
                ...
                p2p[i].SetQueue ("ns3::DropTailQueue", "MaxPackets", UintegerValue(50)); // Buffer of transmission queue
        } 
        // router -- server   
