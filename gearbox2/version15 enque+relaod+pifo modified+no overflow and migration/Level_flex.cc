...
namespace ns3 {
   ...
   int Level_flex::Reload(int k) {
	...
	int npkts;
		...
		int departureRound = tag0.GetDepartureRound();
		...
		setPifoMaxValue(departureRound / term * term+(term-1));

		//cout<<departureRound / term * term+(term-1)<<endl;
		...
		if (re1 != NULL){
			...
			int departureround = tag.GetDepartureRound();	    
	    		...
	    		fifoEnque(re1, cal_index(departureround), tag.GetFlowNo());
  ...
  int Level_flex::getPreviousIndex(){
   ...
   void Level_flex::setPifoReloadHold(int reload_hold) {        

	pifo.SetReloadHold(reload_hold);    
