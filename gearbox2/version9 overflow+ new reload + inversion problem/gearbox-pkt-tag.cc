...
GearboxPktTag::Print (std::ostream &os) const
{
  ...
  os << "departureRound=" << departureRound << " index=" << index;
}
...
GearboxPktTag::GearboxPktTag (int round, int ind, float time, int id)
  : Tag (),
    departureRound (round), index(ind), enquetime(time), uid(id)
{ //cout<<"tag"<<time<<" "<<enquetime;
  NS_LOG_FUNCTION (this << departureRound << " " << index<< ""<< enquetime);
}
