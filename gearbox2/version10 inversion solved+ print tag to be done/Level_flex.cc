...
namespace ns3 {
	...
	////cout << "PE= departureRound:" << departureRound << " getPifoMaxValue():" << getPifoMaxValue() << " pifo.Size():" << pifo.Size() << " pifo.LowestSize():" << pifo.LowestSize() << endl;
	    ...
	    if((tag.GetUid() == 8375)|(tag.GetUid() == 8508)|(tag.GetUid() == 800)){
		...
		cout<<"~~~~~"<<cal_index(departureround)<<endl;
   ...
   int Level_flex::Reload(int k) {
	...
	int npkts;
		...
		if((tag0.GetUid() == 8375)|(tag0.GetUid() == 8508)|(tag0.GetUid() == 8371)){
		...
		if(re1!=0){
			...
			if((tag2.GetUid() == 8371)){
				...
				cout<<tag2.GetUid()<<" re "<<tag2.GetIndex();
