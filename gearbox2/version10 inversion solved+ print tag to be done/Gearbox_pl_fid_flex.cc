...
namespace ns3 {
	...
	if(hasTag == 0){
		...
		//level 1
			...
			if((tag.GetUid() == 800)){
				...
				cout<<tag.GetUid()<<" L1 depart"<<tag.GetDepartureRound()<<" Fifo per"<<FIFO_PER_LEVEL<<" set index "<<departureRound / FIFO_PER_LEVEL % FIFO_PER_LEVEL<<endl;
		...
		//level 0
			...
			if((tag.GetUid() == 800)){
				...
				cout<<tag.GetUid()<<" L0 depart"<<tag.GetDepartureRound()<<" Fifo per"<<FIFO_PER_LEVEL<<" set index "<<departureRound % FIFO_PER_LEVEL<<endl;
		...
		//level 1
			...
			if((tag.GetUid() == 8371)){
				...
				cout<<tag1.GetUid()<<" L1 depart"<<tag1.GetDepartureRound()<<" set index "<<departureRound / FIFO_PER_LEVEL % FIFO_PER_LEVEL<<"tag index "<<tag1.GetIndex()<<endl;
	...
	if(re != 0){//?if(re != 0)
		...
		if(overflow == 2){
			...
			re = levels[level].fifoEnque(re, tag.GetIndex());
   ...
   void Gearbox_pl_fid_flex::Reload(){
	...
	if(currentRound >=540&& currentRound<=556){
