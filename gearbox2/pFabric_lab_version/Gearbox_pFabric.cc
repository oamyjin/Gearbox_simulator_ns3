...
namespace ns3 {
    NS_LOG_COMPONENT_DEFINE("Gearbox_pFabric");
    NS_OBJECT_ENSURE_REGISTERED(Gearbox_pFabric);

    Gearbox_pFabric::Gearbox_pFabric() :Gearbox_pFabric(DEFAULT_VOLUME) {
    }

    Gearbox_pFabric::Gearbox_pFabric(int volume) {
    ...
    void Gearbox_pFabric::setCurrentRound(int cr) {
    ...
    void Gearbox_pFabric::setPktCount(int pktCount) {
        this->pktCount = pktCount;
    }


    int Gearbox_pFabric::RankComputation(QueueDiscItem* item, Flow_pl* currFlow){
    ...
    string Gearbox_pFabric::getFlowLabel(Ptr<QueueDiscItem> item){
    ...
    Flow_pl* Gearbox_pFabric::getFlowPtr(string flowlabel) {
		...
		if (re != 0) {
			...
			if (re != 0){
				...
				if (tag1.GetDepartureRound() != departureRound){
					...
					this->enqueMap[getFlowPtr(getFlowLabel(re))->getFlowNo()] = 0;
			...
			// debug
			/*
			cout<<"enque"<<departureRound<<endl;
				levels[0].pifoPrint();
				for (int i = 0; i < FIFO_PER_LEVEL; i++){
					levels[0].fifoPrint(i);
				}*/
    ...
    void Gearbox_pFabric::AddTag(int flowid, int departureRound, int uid, QueueDiscItem* item){
	GearboxPktTag tag;
    ...
    Ptr<QueueDiscItem> Gearbox_pFabric::DoDequeue() {
	// no more pkts in GB
...
/*
   Ptr<QueueDiscItem> Gearbox_pFabric::FifoDequeue(int earliestLevel) {
...
*/
   ...
   Ptr<QueueDiscItem> Gearbox_pFabric::PifoDequeue() {
   ...
   void Gearbox_pFabric::Reload(){
    ...
    void Gearbox_pFabric::setDropCount(int count){
	this->dropCount = count;
    }


    int Gearbox_pFabric::getDropCount(){
	return dropCount;
    }


    Flow_pl* Gearbox_pFabric::insertNewFlowPtr(string fid, int fno, int weight, int flowsize, vector<int> packet_List,int burstness) {
    ...
    int Gearbox_pFabric::updateFlowPtr(int departureRound, string fid, Flow_pl* flowPtr) {
        string key = convertKeyValue(fid); 
	// update flow info
    ...
    string Gearbox_pFabric::convertKeyValue(string fid) { 
        stringstream ss;
        ss << fid; 
        string key = ss.str();
    ...
    Ptr<QueueDiscItem> Gearbox_pFabric::DoRemove(void) {
        return 0;
    }


    Ptr<const QueueDiscItem> Gearbox_pFabric::DoPeek(void) const {
        return 0;
    }



    Gearbox_pFabric::~Gearbox_pFabric()
    {
        NS_LOG_FUNCTION(this);
    }
    ...
    TypeId Gearbox_pFabric::GetTypeId(void)
    {
        static TypeId tid = TypeId("ns3::Gearbox_pFabric")
            .SetParent<QueueDisc>()
            .SetGroupName("TrafficControl")
            .AddConstructor<Gearbox_pFabric>();
    ...
    bool Gearbox_pFabric::CheckConfig(void)
    {
        NS_LOG_FUNCTION(this);
        if (GetNQueueDiscClasses() > 0){
            NS_LOG_ERROR("Gearbox_pFabric cannot have classes");
            return false;
        }

        if (GetNPacketFilters() != 0){
            NS_LOG_ERROR("Gearbox_pFabric needs no packet filter");
            return false;
        }

        if (GetNInternalQueues() > 0) {
            NS_LOG_ERROR("Gearbox_pFabric cannot have internal queues");
            return false;
        }
    ...
    void Gearbox_pFabric::InitializeParams(void)
    {
        NS_LOG_FUNCTION(this);
    }


    void Gearbox_pFabric::Record(string fname, int departureRound, Ptr<QueueDiscItem> item){
    ...
    void Gearbox_pFabric::RecordFlow(string flowlabel, int departureRound, int uid,string filename){
	string path = "GBResult_pFabric/pktsList/";
    ...
    void Gearbox_pFabric::RecordFCT(string filename, int index,double FCT){
