...
{   
  ...
  if (++m_packetsSent <= m_nPackets)
    {
      //cout<<"m_packetsSent "<<m_packetsSent<<" m_nPackets"<<m_nPackets<<endl;
      ScheduleTx ();
    }
}
...
void ScheduleFlowInputs(){
	while (flow_input.idx < flow_num && Seconds(flow_input.start_time) == Simulator::Now()){
		...
		// Server
                ...
                // trace cwnd
                /*stringstream path;
                path << "GBResult_pFabric/plotResult/" << "flow" << flow_input.idx << ".cwnd";
...
ThroughputMonitor (FlowMonitorHelper* fmhelper, Ptr<FlowMonitor> monitor)
{
 	...
 	Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow (i->first);
        ...
        if (t.destinationAddress == Ipv4Address(dstip.str().c_str())){
                        if(k_fi_pair.find(k) == k_fi_pair.end()){
                                k_fi_pair[k] = fi;
                                std::ofstream ss_d("GBResult_pFabric/pair", std::ios::out | std::ios::app);
                        ...
                        //stringstream path;
                        //path << "GBResult_pFabric/performance" << src_No << ".dat"; //plotResult
  ...
  stringstream path;
  path << "GBResult_pFabric/performance.dat"; //plotResult
  stringstream path2;
  path2 << "GBResult_pFabric/performance_detail.dat"; //plotResult
...
main (int argc, char *argv[])
{
        ...
        // MSS
        Config::SetDefault ("ns3::TcpSocket::ConnCount", UintegerValue(100)); //max retries of syn pkt retransmission
	...
	begint = clock();
        ...
        flowf.open("scratch/traffic_pFabric.txt"); //traffic_test.txt //traffic_fct_20 //traffic_load50
	...
	flowf >> flow_num;
        ...
        tch.SetRootQueueDisc ("ns3::SpPifo_pFabric");
        //tch.SetRootQueueDisc ("ns3::Fifo");
        //tch.SetRootQueueDisc ("ns3::FifoPifo");
        //tch.SetRootQueueDisc ("ns3::Gearbox_pFabric");
        //tch.SetRootQueueDisc ("ns3::AFQ_pFabric");
	...
	// maintain port number for each host pair
        ...
        p2p[CLIENT_CNT + SERVER_CNT - 1].EnableAsciiAll(ascii.CreateFileStream("GBResult_pFabric/plotResult/RouterTCPTopo.tr"));
        p2p[0].EnableAsciiAll(ascii.CreateFileStream("GBResult_pFabric/plotResult/1TCPTopo.tr"));
        p2p[1].EnableAsciiAll(ascii.CreateFileStream("GBResult_pFabric/plotResult/2TCPTopo.tr"));
        p2p[2].EnableAsciiAll(ascii.CreateFileStream("GBResult_pFabric/plotResult/3TCPTopo.tr"));
        p2p[3].EnableAsciiAll(ascii.CreateFileStream("GBResult_pFabric/plotResult/4TCPTopo.tr"));*/

        // record drop
        /*PcapHelper pcapHelper;
        Ptr<PcapFileWrapper> file = pcapHelper.CreateFile ("GBResult_pFabric/plotResult/RouterTCPtopo.pcap", std::ios::out, PcapHelper::DLT_PPP);
        ...
        Ptr<PcapFileWrapper> file2 = pcapHelper2.CreateFile ("GBResult_pFabric/plotResult/1TCPtopo.pcap", std::ios::out, PcapHelper::DLT_PPP);
        devices[0].Get(1)->TraceConnectWithoutContext ("PhyRxDrop", MakeBoundCallback (&RxDrop, file2));
        */

        /*for(uint32_t i=0; i<4; i++)  
                p2p[i].EnablePcapAll("GBResult_pFabric/mytest"); 
        */
	...
	NS_LOG_INFO("Done.");
        ...
        for(int i = 0; i < int(flow_num); i++) { 
                ...
                std::ofstream ss_d("GBResult_pFabric/debug", std::ios::out | std::ios::app);
                ss_d << i+1 << "  s:" << startRxTime[i] << " f:" << lastRxTime[i] << std::endl;
                std::ofstream ss_fct("GBResult_pFabric/flowFCT", std::ios::out | std::ios::app);
                ss_fct << i+1 << "   " << fct << std::endl;
                std::ofstream ss_thr("GBResult_pFabric/flowAvgThr", std::ios::out | std::ios::app);
                ss_thr << i+1 << "   " << (8 * PKTSIZE * flowSize[i]) / (1024 * 1024 * fct) << std::endl;
                std::ofstream ss_tx("GBResult_pFabric/flowPktsTx", std::ios::out | std::ios::app);
                ss_tx << i+1 << "   " << flowPktsTx[i] << std::endl;
                std::ofstream ss_rx("GBResult_pFabric/flowPktsRx", std::ios::out | std::ios::app);
