...
ThroughputMonitor (FlowMonitorHelper* fmhelper, Ptr<FlowMonitor> monitor)
{
 	...
 	Ipv4FlowClassifier::FiveTuple t = classifier->FindFlow (i->first);
        ...
        if (t.destinationAddress == Ipv4Address(dstip.str().c_str())){
		        ...
		        //thr0 <<  curTime.GetSeconds () << " Trx:" << localThrou << " rxBytes:" << i->second.rxBytes << "  Ttx:" << localThroutx << " txBytes:" << i->second.txBytes << " drop:" << i->second.txPackets - i->second.rxPackets << std::endl;
                        thr0 <<  curTime.GetSeconds () << " " << localThrou << std::endl;
  ...
  if (totalRxPkts != 0){
        //thr << curTime.GetSeconds () << "s  throuput:" << totalLocalThrou << " tx:" << totalLocalThroutx << "  totalLostPkts:" << (totalTxPkts-totalRxPkts) << "  avg_delay:" << delaySum.GetSeconds() / totalRxPkts << std::endl;
       thr << curTime.GetSeconds () << " " << totalLocalThrou << std::endl;
  }
  else{
        //thr << curTime.GetSeconds () << "s  throuput:" << totalLocalThrou << " tx:" << totalLocalThroutx << "  totalRxPkts:" << totalRxPkts << "  totalLostPkts:" << (totalTxPkts-totalRxPkts) << std::endl;
        thr << curTime.GetSeconds () << " " << totalLocalThrou << std::endl;
  }
...
main (int argc, char *argv[])
{
	...
	flowf >> flow_num;
        ...
        for (int i = 0; i < CLIENT_CNT; i++){      
                ...
                p2p[i].SetQueue ("ns3::DropTailQueue", "MaxPackets", UintegerValue(1000)); // Buffer of transmission queue
        } 
        // router -- server   
