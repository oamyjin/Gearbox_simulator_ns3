...
namespace ns3 {
   ...
   bool Gearbox_pl_fid_flex::PifoEnqueue(int level, QueueDiscItem* item){
	...
	if(earliestLevel == -1){
		...
		thr <<  Simulator::Now().GetSeconds() <<"uid "<<uid<< " drop " << getDropCount() << " enque " << enqueCount<< " deque "<< dequeCount << " overflow " << overflowCount << " reload "<< reloadCount <<"migration"<<migrationCount<< std::endl;
