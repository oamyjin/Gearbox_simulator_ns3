...
*/
	...
	// coutMark
	if(currentRound >=3530&& currentRound<=3550){
	...
	if(levels[0].getPifoSize() == 0){
		...
		thr <<  Simulator::Now().GetSeconds() << " drop " << getDropCount() << " TOTAL:" << total << " fifoenque:" << levels[0].getFifoEnque() << " fifodeque:" << levels[0].getFifoDeque()
			...
			<< " migration:" << migrationCount << " overflow:" << overflowCount << " uid:" << maxUid << std::endl;
   ...
   void Gearbox_pl_fid_flex::Reload(){
	...
	if(levels[j].getReloadHold() == 1){
			...
			//mark
			if(currentRound >=3530&& currentRound<=3550){
			 	cout << " levels[j].getPifoMaxValue():" << levels[j].getPifoMaxValue() << endl;
			}		
								

			//mark
			if(currentRound >=3530&& currentRound<=3550){
				cout << "After levels[j].getReloadHold() :" << levels[j].getReloadHold() << " max:" << levels[j].getPifoMaxValue() << endl;
			}
		}
