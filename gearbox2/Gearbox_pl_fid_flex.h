...
namespace ns3 {
    ...
    private:
        ...
        static const int STEP_DOWN_FIFO = 4;         // 01212020 Peixuan flex level
    ...
    public:
        ...
        */
        ...
        //if the earliest packet is in the pifo, return true. Otherwise return false.
        bool earliestPacketInPifo();

        void pifoEnque(int level, Ptr<QueueDiscItem> item);
        sendBackToFifo(int level);
        bool isHigherThanH();


        //if the earliest packet is in the pifo, pifo deque once.
        void pifodeque(int level);
        //>0, return L - pifo[level]
        int isLowerThanL(int level);
        void reloadpifo(int level, int packets);
        findearliestfifo();
