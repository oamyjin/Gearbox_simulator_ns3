...
namespace ns3 {
	...
	if(hasTag == 0){
		...
		RecordFlow(flowlabel,departureRound,uid);
			...
			RecordDropInfo(currentRound, item, "too large rank"); // record: vt, dp, reason
			...
			RecordDropInfo(currentRound, item, "too burst"); // record: vt, dp, reason
	...
	if(re != 0){//?if(re != 0)
		...
		if (level == DEFAULT_VOLUME - 1){
			...
			RecordDropInfo(currentRound, re, "L2 overflow"); // record: vt, dp, reason
		...
		if(overflow == 2){
			...
			RecordDropInfo(currentRound, re, "no more overflow chance"); // record: vt, dp, reason
	...
	if(re != 0){//?if(re != 0)
		...
		if (level >= DEFAULT_VOLUME - 1){
			...
			RecordDropInfo(currentRound, re, "L2 overflow fifoenque"); // record: vt, dp, reason
		...
		if(overflow == 2){
			...
			RecordDropInfo(currentRound, re, "no more overflow chance fifoenque"); // record: vt, dp, reason
   ...
   void Gearbox_pl_fid_flex::Migration(int currentRound){
    ...
    Flow_pl* Gearbox_pl_fid_flex::insertNewFlowPtr(uint64_t flabel, int fno, int weight, int brustness) {



	Flowlist.push_back(flabel);



        string key = convertKeyValue(flabel);



        Flow_pl* newFlowPtr = new Flow_pl(flabel, fno, weight, brustness);
	...
	void Gearbox_pl_fid_flex::RecordDropInfo(int vt, Ptr<QueueDiscItem> item, string reason){
		GearboxPktTag tag;
		...
		FILE *fp;
		fp = fopen("GBResult/pktsList/dropRecord", "a+"); //open and write
		fprintf(fp, "%f", Simulator::Now().GetSeconds());
		fprintf(fp, "\t%s", "vt:");
		fprintf(fp, "%d", vt);
		fprintf(fp, "\t%s", "departureRound:");
		fprintf(fp, "%d", dp);
		fprintf(fp, "\t%s", "uid:");
		fprintf(fp, "%d", uid);
		fprintf(fp, "\t%s", "reason:");
		fprintf(fp, "%s", reason.c_str());
		fprintf(fp, "\n");
		fclose(fp);


		FILE *fp1;
		string path = "GBResult/pktsList/dropRecord";
		path.append(to_string(getFlowPtr(getFlowLabel(item))->getFlowNo()));
		fp1 = fopen(path.data(), "a+"); //open and write
		fprintf(fp1, "%f", Simulator::Now().GetSeconds());
		fprintf(fp1, "\t%d", 0);
		fprintf(fp1, "\n");
		fclose(fp1);
	}
	...
	void Gearbox_pl_fid_flex::Record(string fname, int departureRound, Ptr<QueueDiscItem> item){
		...
		// record total packets in the GB
		int totalPktsGB = levels[0].getFifoTotalNPackets() + levels[1].getFifoTotalNPackets() + levels[1].getPifoSize() + levels[2].getFifoTotalNPackets() + levels[2].getPifoSize();
		
		FILE *fp0;
		fp0 = fopen("GBResult/pktsList/TotalGBPktNSize", "a+"); //open and write
		fprintf(fp0, "%f", Simulator::Now().GetSeconds());
		fprintf(fp0, "\t%d", totalPktsGB);
		fprintf(fp0, "\n");
		fclose(fp0);
		...
		for (int i = 0; i < FIFO_PER_LEVEL; i++){
			...
			fprintf(fpl0, "\t%d", levels[0].getFifoNPackets(i));
		...
		for (int i = 0; i < FIFO_PER_LEVEL; i++){
			...
			fprintf(fpl1, "\t%d", levels[1].getFifoNPackets(i));
		...
		for (int i = 0; i < FIFO_PER_LEVEL; i++){
			...
			fprintf(fpl2, "\t%d", levels[2].getFifoNPackets(i));
    ...
    void Gearbox_pl_fid_flex::RecordFlow(uint64_t flowlabel, int departureRound,int uid){
