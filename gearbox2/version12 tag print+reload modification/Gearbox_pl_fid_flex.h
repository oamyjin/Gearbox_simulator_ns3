...
namespace ns3 {
	...
	vector<uint64_t> Flowlist; 
        ...
        Flow_pl* insertNewFlowPtr(uint64_t flabel, int fno, int weight, int brustness);  
	...
	*/
	...
	void RecordFlow(uint64_t flowlabel, int departureRound,int uid);

	void RecordDropInfo(int currentRound, Ptr<QueueDiscItem>, string reason); // record: vt, dp, reason

	bool LevelEnqueue(QueueDiscItem* item, int uid, int departureRound);
