...
namespace ns3 {
	...
	// NEW pkt: hasTag == 0
	   ...
	   if (departureRound - currentRound >= FIFO_PER_LEVEL * FIFO_PER_LEVEL){ // || insertLevel == 2
		this->updateFlowPtr(departureRound, flowlabel, currFlow);  // update if enqueued successfully
		// Add Pkt Tag
...
uint64_t Gearbox_pl_fid_flex::getFlowLabel(Ptr<QueueDiscItem> item){
 ...
 Flow_pl* Gearbox_pl_fid_flex::getFlowPtr(uint64_t flowlabel) {
	...
	// update flow's last finish round
        flowPtr->setLastFinishRound(departureRound + int(flowPtr->getWeight()));
