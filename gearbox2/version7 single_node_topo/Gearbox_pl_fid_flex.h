...
namespace ns3 {
	...
	uint64_t getFlowLabel(Ptr<QueueDiscItem> item);
        Flow_pl* getFlowPtr(uint64_t flowLabel);  

        Flow_pl* insertNewFlowPtr(uint64_t fid, int weight, int brustness);
	...
	/*
	 * input: enqueued packet
	 * return: rank value (departure round in current SFQ scheme)
	*/
