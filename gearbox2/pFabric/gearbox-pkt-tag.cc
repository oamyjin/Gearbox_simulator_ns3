...
GearboxPktTag::Serialize (TagBuffer buf) const
{
  ...
  //32bits
  buf.WriteU32 (departureRound);//4
  buf.WriteU32 (uid);//4
  buf.WriteU16 (flowNo);//2
  buf.WriteU8 (overflow);//1
  buf.WriteU8 (migration);//1
  buf.WriteU8 (reload);//1
  buf.WriteU8 (fifoenque);//1
  buf.WriteU8 (fifodeque);//1
  buf.WriteU8 (pifoenque);//1
  buf.WriteU8 (pifodeque);//1
  buf.WriteU16 (enquetime);//2
  buf.WriteU16 (dequetime);//2
}
void 
GearboxPktTag::Deserialize (TagBuffer buf)
{
  ...
  enquetime = buf.ReadU16();
  dequetime = buf.ReadU16();
}
void 
...
void
GearboxPktTag::SetFlowNo (int flowNo)
{
  NS_LOG_FUNCTION (this << flowNo);
  this->flowNo = flowNo;
}
int
