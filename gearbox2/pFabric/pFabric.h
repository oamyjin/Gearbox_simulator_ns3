...
namespace ns3 {
    ...
    private:
        ...
        static const int FIFO_PER_LEVEL = 25;  // flex level

        static const int DEFAULT_WEIGHT = 2;   // default weight

        static const int DEFAULT_BURSTNESS = 20000;  // default burstness 1000 90000

 	static const int SPEEDUP_FACTOR = 4; // default speed up
	static const int GRANULARITY = 800; //33 3000
	...
	typedef std::map<int, bool> EnqueMap;//record whether each flow is enqueued to scheduler
        EnqueMap enqueMap;
	//typedef std::map<int, string> FlowLabelMap; //corresponding the flowlabel and flow number
        //FlowLabelMap flowLabelMap;
	...
	uint64_t rankDifference[1500] = {0}; // the difference between the current round and the incoming pkt's rank
        ...
        Flow_pl* insertNewFlowPtr(string fid, int fno, int weight,int flowsize, int burstness); 
