...
namespace ns3 {
	...
	cout<<"source"<<ipHeader.GetSource()<<endl;

	cout<<"destination"<<ipHeader.GetDestination()<<endl;
	...
	//cout<< "extracted departureRound"<<departureRound<<endl;
   ...
   void Gearbox_pl_fid_flex::Migration(int currentRound){
	...
	while(!levels[1].isCurrentFifoEmpty() && (i < SPEEDUP_FACTOR)){
		...
		DoEnqueue(item);		
		i++;
		cout << "Migrate packet: "<< departureRound << " from level 1" << endl;
	}
	...
	while(!levels[2].isCurrentFifoEmpty() && (j < SPEEDUP_FACTOR)){
		...
		DoEnqueue(item);
		j++;
		cout<<"Migrate packet: " << departureRound << " from level2 " << endl;
	}
