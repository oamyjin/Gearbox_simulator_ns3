...
if __name__ == "__main__":
	...
	n_flow = 0
	ofile.write("%d \n"%n_flow_estimate)
	host_list = [(base_t + int(poisson(avg_inter_arrival)), i + 1) for i in range(nhost)]
	...
	while len(host_list) > 0:
		...
		if (t + inter_t > time + base_t):
			...
			print(len(src_record))
			size = math.ceil(size / 1052)
			ofile.write("%d %d 1000 %d %.9f\n"%(src, dst, size, t * 1e-9))
...
# python traffic_gen.py -c WebSearch_distribution.txt -n 320 -l 0.3 -b 100G -t 0.1
